...
Result nearestPoints_BF(vector<Point> &vp) {
    ...
    for (int i = 0; i < vp.size(); i++) {
        for (int j = 0; j < vp.size(); j++) {
            if (i != j) {
                actual_dis = vp[i].distance(vp[j]);
                if (actual_dis < res.dmin) {
                    res.dmin = actual_dis;
                    res.p1 = vp[i];
                    res.p2 = vp[j];
                    if (res.dmin == 0)
...
static Result np_DC(vector<Point> &vp, int left, int right, int numThreads) {
    ...
    if(numThreads>1){
        ...
        thread t([&](){resL=np_DC(vp,left,mid,numThreads/2);});
        //resL = np_DC(vp, left, mid, numThreads);
        resR = np_DC(vp, mid + 1, right, numThreads/2);
        t.join();
        res = resL.dmin < resR.dmin ? resL : resR;

        // Determine the strip area around middle point


        int midX = (vp[mid].x + vp[mid + 1].x) / 2;
        int Sleft = mid, Sright = mid + 1;
        ...
        // Select the best solution from left and right
        resL = np_DC(vp, left, mid, numThreads);
        resR = np_DC(vp, mid + 1, right, numThreads);
        res = resL.dmin < resR.dmin ? resL : resR;
