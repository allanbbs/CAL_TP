            ...
            if (numbers[i][j] == 0) {
                for (int n = 1; n < 10; n++) {
                    if (is_possible(i, j, n)) {
                        ...
                        lineHasNumber[i][n] = true;
                        columnHasNumber[j][n] = true;
                        block3x3HasNumber[i][j][n] = true;
                        solve();
                        numbers[i][j] = 0;
                        countFilled--;
                        columnHasNumber[j][n] = false;
                        lineHasNumber[i][n] = false;
                        block3x3HasNumber[i][j][n] = false;
                    }
                }
                return false;
            }
...
bool Sudoku::is_possible(int x, int y, int n) {
    if (columnHasNumber[y][n])
        return false;
    if (lineHasNumber[x][n])
        return false;
    return !block3x3HasNumber[x][y][n];
}
